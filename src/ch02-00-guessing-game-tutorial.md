...
`Result` 타입은 [*열거형 (enumerations)*][enums]<!-- ignore --> 으로서
...
실행 해 보세요. 프로그램이 우리의 지시에 정확히 따르다보니 새로운 문제가
...
`expect` 메소드 호출을 `match` 표현식으로 바꾸는 것은 에러 발생 시 종료되지 않게
처리하는 일반적인 방법입니다. `parse` 메소드가 `Result` 타입을
