...
조건을 제공한 다음 다음과 같이 기술하는 식이죠. “만약 조건을 만족하면,
이 코드 블럭을 실행하세요. 그렇지 않다면 코드 블럭을 실행하지 마세요."
...
중괄호로 된 블록에 배치됩니다.
`if` 표현식의 조건과 관련된 코드 블럭은
2장의 [“추측값과 비밀번호 비교하기”][comparing-the-guess-to-the-secret-number]<!-- ignore -->절에서
...
참일때의 첫번쨰 본문을 실행합니다. 6이 2로 나누어 떨어지지만
