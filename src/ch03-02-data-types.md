...
> 프로그램이 종료되는 경우 패닉이라는 용어를 사용합니다; 9장의  [“`panic!` 을
> 이용한 복구 불가능한 에러”][unrecoverable-errors-with-panic]<!-- ignore -->
...
[“제어 흐름”][control-flow]<!-- ignore --> 절에서 다루겠습니다.
...
[“스트링을 이용하여 UTF-8로 인코딩된 텍스츠 저장하기”][strings]<!-- ignore -->
...
변수를 만듭니다. 대부분의 언어가 그렇듯이 튜플의 첫 번쨰 인덱스은
0입니다. 
...
(힙과 스택은 4장에서 더 다루겠습니다) 항상 고정된 갯수의 원소로
...
예제를 다음처럼 컴파일은 되지만 바꾸면 실행 시에 에러가 발생하며
멈추게 됩니다:
